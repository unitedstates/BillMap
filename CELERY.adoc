...
The repository here provides a scraper for metadata and xml:
https://github.com/unitedstates/congress

It creates a directory tree as follows:
...
- Difference between document.xml and data.xml

data.xml corresponds to the data.json at the same level that is the metadata.
...
Celery tasks updates uscongress bill and metadata at regular intervals (at this moment, it runs daily at midnight).
...
It will creates `data.json` out of `data.xml` and text_versions.
...
After completing celery task `bill_data_task`, another celery task `process_bill_meta_task` runs.

4. Create Related bills.

Once the celery task above is finished, other celery task named `related_bill_task` runs.
...
In the feature, we may have to distinguish.
