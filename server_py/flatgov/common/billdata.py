#!/usr/bin/env python3
#
# Command line template from
# https://gist.githubusercontent.com/opie4624/3896526/raw/3aff2ad7030a74ce26f9fcf80791ae0396d84f18/commandline.py
import gzip
import json
import logging
import os
...
def walkBillDirs(
        rootDir=constants.PATH_TO_CONGRESSDATA_DIR, processFile=logName,
        dirMatch=getTopBillLevel, fileMatch=isDataJson
):
...
def getCosponsors(fileDict: Dict, includeFields=[]) -> list:
    """
    Gets Cosponsors from data.json Dict. `includeFields` is a list of keys to keep.
    The most useful are probably 'name' and 'bioguide_id'.

    Args:
        fileDict (Dict): the Dict created from data.json
        includeFields (list): the fields in the cosponsor object to keep.
        If no 'includeFields' list is provided, all fields are preserved.
...
def getBillTitles(fileDict: Dict, include_partial=True, billType='all') -> list:
    """
    Get a list of bill titles. If include_partial = True (default), gets all titles.
    Otherwise, gets only titles that correspond to the whole bill.

    Args:
        fileDict (Dict): the Dict created from data.json
        include_partial (bool, optional): Include titles for part of the bill. Defaults to True.
        billType (str, optional): Filter by billType (e.g. 'ih', 'rh', etc.)
        Defaults to 'all', which does not filter.
...
def updateBillsMeta(billsMeta={}):
    def addToBillsMeta(dirName: str, fileName: str):
        ...
        billsMeta[billCongressTypeNumber]['titles_whole_bill'] = [title.get('title') for title in titles if
                                                                  not title.get('is_for_portion')]
...
def addTitleMainToRelated(dirName: str, fileName: str):
    try:
        ...
        billsRelatedByMainTitle = [billnumber for billnumber, billdata in relatedDict.items() if
                                   "bills-title_match_main" in billdata.get("reason").split(", ")]
        ...
        for bill in billsRelatedByMainTitle:
            if not relatedDict_old.get(bill):
                ...
                relatedDict_old[bill]["reason"] = ", ".join(
                    list(set((relatedDict_old[bill]["reason"] + ", bills-title_match_main").split(", "))))
...
# This  using the data in json files created by the Go functions
# To update by Congress, set rootDir to
