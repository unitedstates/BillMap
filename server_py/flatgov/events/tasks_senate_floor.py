...
def process_xml_senate_floor(source):
    ...
    for meeting in meetingBag:
        ...
        #    <full_text>12:00 p.m.: Convene and resume consideration of the veto message on <measure>h6395</measure>, the National Defense Authorization Act for Fiscal Year 2021.</full_text>
        #</convene>
        ...
        description = "{} Congress {} Session {} - Convene Notes=[{}] Adjourn Notes=[{}]".format(year, congress, session, conveneFullText, adjournFullText)

        if not adjournTime:
            adjournTime = str(int(conveneTime) + 100)

        start = set_eastern_timezone(datetime.datetime.strptime("{} {} {} {}".format(conveneMonth, conveneDate, year, conveneTime), "%m %d %Y %H%M"))
        end = set_eastern_timezone(datetime.datetime.strptime("{} {} {} {}".format(conveneMonth, conveneDate, year, adjournTime), "%m %d %Y %H%M"))
