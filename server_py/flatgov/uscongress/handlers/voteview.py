...
from uscongress.handlers.vote_info import output_vote
...
def run(options):
    ...
    session_dates = list(csv.DictReader(
        io.StringIO(utils.download("http://www.govtrack.us/data/us/sessions.tsv").encode("utf8")), delimiter="\t"
    ))
...
def parse_vote_list_file(vote_list_file):
    ...
    for vote_list_line in vote_list_file.split("\r\n"):
        ...
        vote_info["state"] = get_state_from_icpsr_state_code(
            vote_info["icpsr_state"]
        ) if vote_info["icpsr_state"] is not None else None
        vote_info["party"] = get_party_from_icpsr_party_code(
            vote_info["icpsr_party"]
        ) if vote_info["icpsr_party"] is not None else None
        ...
        try:
            bioguide_id = utils.get_person_id(
                "icpsr" if vote_info["state_name"] != "USA" else "icpsr_prez", icpsr_id, "bioguide"
            )
            ...
            if icpsr_id not in (99999, 10509, 15067) and vote_info["state_name"] != "USA":
                logging.error("Problem with member %s ([%d] %s) of %s %s: %s" % (
            ...
            logging.debug("Parsed member %s ([%d] %s) of %s %s..." % (
                vote_info["member_name"],
                vote_info["icpsr_party"],
                vote_info["party"],
                vote_info["state_name"],
...
def parse_rollcall_dtl_list_file(rollcall_dtl_list_file, congress):
    ...
    for rollcall_dtl_list_line in rollcall_dtl_list_file.split("\r\n"):
        ...
        rollcall_dtl_list_line_info = extract_rollcall_info_from_parsed_rollcall_dtl_list_line(
        ...
        if rollcall_dtl_list_line_info["line"] == 1:
            ...
            if m:
                bill_type_map = {
                    ...
                    'SCR': 'sconres', 'SCRE': 'sconres', 'SCRES': 'sconres', 'SCONRES': 'sconres', 'SCONR': 'sconres',
                    'SCON': 'sconres',
                    'HRE': 'hres', 'HRES': 'hres',
...
def parse_rollcall_description(rollcall):
    ...
    while len(dparts) > 1:
        # remove trailing spaces, hyphens, and periods (which occur at the end of the final dpart
        # but not inner ones because it is the split string)
...
def get_votes(chamber, congress, options, session_dates):
    ...
    rollcall_list_file = utils.download(
        rollcall_list_url,
        cache_file_for(congress, chamber, "dtl"),
        options
    ).encode("utf-8")
    ...
    for i in range(min(rollcall_list)+1, max(rollcall_list) - 1):
        ...
        # Lump the vote with the previous date.
            ...
            rollcall_list[i]["date"] = rollcall_list[i - 1]["date"]
            logging.error("Replacing %s with %s (but might be as late as %s)." % (
    ...
    for rollcall_number in rollcall_list:
        ...
        if not rollcall["date"]:
            logging.error("Vote on %s was an invalid date, so we can't determine the session to save the file.. | %s" % (
                rollcall["date_unparsed"],
                rollcall["description"]
            ))
            continue
